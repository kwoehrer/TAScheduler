...
View State Page
GIVEN The user is an Instructor and is logged in and at the Instructor View State page
THEN all profile information should be correctly displayed
-----------------------------------------------------------------------
'''


class TestInstrViewState(TestCase):
    def setUp(self):
        self.client1 = Client()
    ...
    def testAdminProfileInfo(self):
        response = self.client1.get('/ProfileStates/InstrViewState')
    ...
    def testInstructorProfileInfo(self):
        response = self.client2.get('/ProfileStates/InstrViewState')
        self.assertContains(response, self.instructor.getFirstName())
        self.assertContains(response, self.instructor.getLastName())
        self.assertContains(response, self.instructor.getUserType())
        self.assertContains(response, self.instructor.getEmail())
    ...
    def testTAProfileInfo(self):
        response = self.client3.get('/ProfileStates/InstrViewState')
    ...
    def test_InstrViewState_to_SearchPage(self):

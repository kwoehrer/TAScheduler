...
class TestAddress(TestCase):
    ...
    def testGoodFirstName(self):
        with self.assertRaises(ValueError, msg="incorrect user type"):
            self.good_account_attributes['user_type'] = 'TA'
        self.assertEqual(self.good_account_attributes['first_name'], 'John', msg='incorrect number')

    def testGoodLastName(self):
        with self.assertRaises(ValueError, msg="incorrect user type"):
            self.good_account_attributes['user_type'] = 'TA'
        self.assertEqual(self.good_account_attributes['last_name'], 'Doe', msg='incorrect number')

    def testBadFirstName(self):
        with self.assertRaises(ValueError, msg="incorrect user type"):
            self.good_account_attributes['user_type'] = 'TA'
        length = len(self.good_account_attributes['first_name'])
        self.assertEqual(length, 24, msg='Bad length')

    def testBadLastName(self):
        with self.assertRaises(ValueError, msg="incorrect user type"):
            self.good_account_attributes['user_type'] = 'TA'
        length = len(self.good_account_attributes['last_name'])
        self.assertEqual(length, 24, msg='Bad length')

    def testBadFirstName(self):
        with self.assertRaises(ValueError, msg="incorrect user type"):
            self.good_account_attributes['user_type'] = 'TA'
        self.assertNotEqual(self.good_account_attributes['first_name'], -1, msg="good type")

    def testBadLastName(self):
        with self.assertRaises(ValueError, msg="incorrect user type"):
            self.good_account_attributes['user_type'] = 'TA'
        self.assertNotEqual(self.good_account_attributes['last_name'], -1, msg="good type")

    def BadType2(self):
        with self.assertRaises(ValueError, msg="incorrect user type"):
            self.good_account_attributes['user_type'] = 'TA'
        with self.assertRaises(TypeError, msg="incorrect user type"):
            if re.match("^[A-Za-z]*$", self.good_account_attributes['first_name']) is not None:
                return True
            return False
    ...
    def BadType3(self):
        with self.assertRaises(ValueError, msg="incorrect user type"):
            self.good_account_attributes['user_type'] = 'TA'
        with self.assertRaises(TypeError, msg="incorrect user type"):
            if re.match("^[A-Za-z]*$", self.good_account_attributes['last_name']) is not None:
                return True
            return False

    def testExistingFirstName(self):
        self.good_account_attributes['phone_number'] = 'newemail@aol.com'
        with self.assertRaises(ValueError,
                               msg='Cannot create an existing email'):
            self.acc.setPhoneNumber(self.ta.getFirstName())

    def testExistingLastName(self):
        self.good_account_attributes['phone_number'] = 'newemail@aol.com'
        with self.assertRaises(ValueError,
                               msg='Cannot create an existing email'):
            self.acc.setPhoneNumber(self.ta.getLastName())
